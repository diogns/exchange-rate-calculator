...
pipeline {
  ...
  environment {
    ...
    PROJECT_GIT_SOURCE = 'git.auna.pe/[...].git'
    CONFLUENCE_WIKI_URL = 'https://aunadigital.atlassian.net/wiki'
  ...
  stages {
    ...
    stage('Scan security repository'){
      steps {
        script{
          docker.image('zricethezav/gitleaks:latest').inside('-u gitleaks --entrypoint \'\'') {
    ...
    stage('Docs: Changelog'){
      ...
      steps {
        script {
          withCredentials([usernamePassword(credentialsId: 'gitlab-credential-user', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PWD')]) {
            sh (returnStdout: false, script: "yarn release", label: "Generating CHANGELOG ...")
    ...
    stage('Docs: Publish .md'){
      ...
      steps {
        script {
          ...
          docker.image('587279634644.dkr.ecr.us-east-1.amazonaws.com/mkdocs/confluence')
            .inside('-u node -e CONFLUENCE_USER_TOKEN=${CONFLUENCE_USER_TOKEN} -e CONFLUENCE_USER=${CONFLUENCE_USER} -e PROJECT_NAME=${PROJECT_NAME} -e CONFLUENCE_WIKI_URL=${CONFLUENCE_WIKI_URL} --env-file=${workspace}/.env --entrypoint \'\'') {
              withAWS(credentials: 'aws-devops-iac', region:'us-east-1') {
